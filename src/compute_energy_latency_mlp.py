...
def get_nc_size (net, xbar_size, num_xbar_per_pe):
    num_pe = 0
    for i in range (len(net)-1):
        num_xbar = ceil(net[i]/xbar_size) * ceil(net[i+1]/xbar_size)
        num_pe = num_pe + ceil(num_xbar/num_xbar_per_pe)
    return ceil(sqrt(num_pe))


# MLP configurations
mlp_net = [[784, 800, 800, 10],
       [1024, 1200, 1200, 10],
       [1024, 1200, 1200, 1200, 10]]
...
# noc packet latency for corresponding NC sizes of networks
...
num_inter = []
nc_size = []
num_hop_max = [] #layer-wise max hop of each net

for j in range(len(mlp_net)):
    # print ('computing energy for net: ', mlp_net[j])
    num_compute.append(0)
    num_intra.append(0)
    num_inter.append(0)
    nc_size_temp = get_nc_size (mlp_net[j], xbar_size, num_xbar_per_pe)
    nc_size.append(nc_size_temp)
    num_hop_max_net = []

    for i in range(len(mlp_net[j])-1):
        #print ('computing layer ' + str(i) + '......')

        # number of compute operations
        logical_xbar_size = [mlp_net[j][i], mlp_net[j][i+1]]
        ...
        assert (mlp_net[j][i] <= max_fan_in), 'neuron fan-in exceeds satisfy zero hop constraint'
        num_intra[j] = num_intra[j] + ceil(log(ceil(logical_xbar_size[0]/xbar_size), 2)) *  ceil(logical_xbar_size[1]/xbar_size)
        ...
        if (i != 0):
            num_xbar_prev = ceil (mlp_net[j][i-1]/xbar_size) * ceil (mlp_net[j][i]/xbar_size)
            num_in_col = ceil(num_xbar_prev/ (num_xbar_per_pe*nc_size_temp))
        else:
            num_in_col = 1

        max_hop = num_in_col + num_out_col - 1
        num_hop_max_net.append(max_hop)
    ...
    num_hop_max.append(num_hop_max_net)
...
for j in range(len(mlp_net)):
    ...
    num_packets = xbar_size / packet_width
...
for j in range(len(mlp_net)):
    ...
    for i in range(len(mlp_net[j])-1):
        ...
        nc_size_temp = int(ceil((nc_size[j]/2.0))*2)
        packet_lat_per_hop = noc_average_latency[str(nc_size_temp)] / noc_average_hop[str(nc_size_temp)]
        inter_latency_temp = packet_lat_per_hop * num_hop_max[j][i]
